      ...
      <div className="min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8"
        ...
        <div className="w-full max-w-md space-y-8 text-center">
          ...
          <div className="rounded-lg p-5 text-left space-y-3"
            style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-subtle)' }}>
            <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>What to do next</p>
            <ul className="space-y-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
              {TIPS.map((tip, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="mt-0.5 h-4 w-4 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-white"
                    style={{ backgroundColor: 'var(--color-forest-green)' }}>
                    {i + 1}
                  </span>
                  {tip}
          ...
          <div className="space-y-3">
            ...
            <Link to="/login" className="block text-sm font-medium transition-colors hover:underline"
      ...
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          <div className="mx-auto h-12 w-12 rounded-lg flex items-center justify-center mb-4"
            style={{ backgroundColor: 'var(--color-forest-green)', border: '1px solid var(--color-border-olive)' }}>
        ...
        {localError && (
          <div className="rounded-lg p-4 border-l-4"
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium" style={{ color: 'var(--error-text)' }}>{localError}</p>
            </div>
          </div>
        )}
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-4 rounded-lg p-6"
            ...
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              We will only send a reset link if this email is registered on the platform.
            </p>
          </div>

          <button type="submit" disabled={forgotMutation.isLoading}
