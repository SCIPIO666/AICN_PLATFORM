...
async function getTrainerSessions(userId, role, filters = {}) {
  ...
  const [sessions, total] = await Promise.all([
    prisma.session.findMany({
      ...
      include: { enrolments: { include: { user: { select: { id: true, name: true, email: true, phone: true } } } }, _count: { select: { enrolments: true } } },
      orderBy: { date: 'asc' }
    }),
    prisma.session.count({ where })
  ]);
  
  return { sessions, total, totalPages: Math.ceil(total / limit), hasNextPage: page < Math.ceil(total / limit), hasPrevPage: page > 1 };
}
...
module.exports = { applyForTrainer, getMyTrainerProfile, updateMyTrainerProfile, withdrawApplication, getAllTrainerApplications, getTrainerApplicationById, approveTrainerApplication, rejectTrainerApplication, deleteTrainerApplication, getTrainerSessions, getApprovedTrainers };